...
import sys, json, random, traceback, pymongo, math
...
class Worker:
    ...
    def _parse(self, collection, q, p, input, output):
        try:
            db = {}
            for record in self.m[self.db][collection].find({input:int(q.id), "DAY":{"$lte":int(q.endAt), "$gte":int(q.startAt)}}):
                key = record[output] if isinstance(output, str) else ','.join([str(record[i]) if i in record else '' for i in output])
                try: db[key]['bid'], db[key]['bidres'], db[key]['push'], db[key]['show'], db[key]['click'], db[key]['cost'], db[key]['selfcost'] = db[key]['bid'] + record['bid'], db[key]['bidres'] + record['bidres'], db[key]['push'] + record['push'], db[key]['show'] + record['show'], db[key]['click'] + record['click'], db[key]['cost'] + record['cost'], db[key]['selfcost'] + record['selfcost']
                except:
                    db[key] = {}
                    db[key]['bid'], db[key]['bidres'], db[key]['push'], db[key]['show'], db[key]['click'], db[key]['cost'], db[key]['selfcost'] = record['bid'], record['bidres'], record['push'], record['show'], record['click'], record['cost'], record['selfcost']
            r = reportResult()
            r.totalSize = len(db)
            r.data = [Response(str(k), db[k]['push'], db[k]['show'], db[k]['click'], db[k]['cost'], db[k]['bid'], db[k]['bidres'], db[k]['selfcost']) for k in sorted(db.keys())[p.pageSize*(p.pageNumber-1):p.pageSize*p.pageNumber]]
            ...
            print str(e)
            self.m, self.db = pymongo.MongoClient("localhost", 12331), 'StatV3'
            r = reportResult()
    ...
    def HostByGid(self, q, p): return self._parse('HostDetail', q, p, "GOURPID", "HOST")
    ...
    def AdspaceByGid(self, q, p): return self._parse('AdspaceDetail', q, p, "GOURPID", "ADSPACE")
    ...
    def getCostByUid(self, q):
        ...
        if R != None and R.data != None and len(R.data) > 0:
            return sum([r.cost for r in R.data])
        else:
            return 0
